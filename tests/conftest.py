...
Pytest fixtures and rootdir marker for this package. An empty
conftest.py at tests/ is the canonical SciTeX convention (audit-project
PS208) — it pins the pytest rootdir and gives downstream fixtures a
home.

In addition, this file wires up subprocess coverage so child Python
interpreters spawned by tests (e.g. `subprocess.run([sys.executable,
str(example)])` in tests/integration/test_examples.py and
tests/examples/test_quickstart.py) actually contribute to coverage.
...
def _ensure_subprocess_coverage_shim() -> None:
    ...
    pth = purelib / "_scitex_compat_subprocess_coverage.pth"
    shim = (
        "import os, coverage\n"
